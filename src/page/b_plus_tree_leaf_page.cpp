...
int LeafPage::KeyIndex(const GenericKey *key, const KeyManager &KM) {
  for (int i = 0; i < this->GetSize(); i++) {
    if (KM.CompareKeys(key, KeyAt(i)) <= 0) {  // key <= KeyAt(i)
      return i;
    }
  }
  return GetSize();
...
int LeafPage::Insert(GenericKey *key, const RowId &value, const KeyManager &KM) {
  ...
  if (index < GetSize()) {
    if (KM.CompareKeys(key, KeyAt(index)) == 0) {  // 已经存在
      return GetSize();
    }
    IncreaseSize(1);
...
void LeafPage::MoveHalfTo(LeafPage *recipient) {
  int half = GetMinSize();
  recipient->CopyNFrom(PairPtrAt(half), GetSize() - half);  // 将this中的后半部分复制到recipient中
  SetSize(half);
}
...
void LeafPage::CopyNFrom(void *src, int size) {
  PairCopy(PairPtrAt(GetSize()), src, size);  // 将src中的size个元素复制到this中
  IncreaseSize(size);
}
