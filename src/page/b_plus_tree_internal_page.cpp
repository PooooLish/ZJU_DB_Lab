...
int InternalPage::InsertNodeAfter(const page_id_t &old_value, GenericKey *new_key, const page_id_t &new_value) {
  int idx = ValueIndex(old_value); // 找到old_value对应的index
  if (idx == -1) return GetSize(); // 如果没找到，返回当前大小
  for (int i = GetSize(); i > idx + 1; i--) { // 从后往前遍历，将idx后面的key和value都往后移动一位
    SetKeyAt(i, KeyAt(i - 1));
    SetValueAt(i, ValueAt(i - 1));
  }
  SetKeyAt(idx + 1, new_key); // 设置新的key和value
  SetValueAt(idx + 1, new_value); // 设置新的key和value
  IncreaseSize(1); // 更新大小
  return GetSize(); // 返回大小
}
...
void InternalPage::MoveHalfTo(InternalPage *recipient, BufferPoolManager *buffer_pool_manager) {
  int half = GetMinSize(); 
  recipient->CopyNFrom(PairPtrAt(half), GetSize() - half, buffer_pool_manager); // 将后半部分的key和value复制到recipient
  SetSize(half); 
}
...
void InternalPage::Remove(int index) {
  for (int i = index; i < GetSize() - 1; i++) { // 从index开始，将后面的key和value都往前移动一位
    SetKeyAt(i, KeyAt(i + 1));
    SetValueAt(i, ValueAt(i + 1));
  }
  IncreaseSize(-1); // 更新大小
...
void InternalPage::MoveLastToFrontOf(InternalPage *recipient, GenericKey *middle_key,
  ...
  recipient->CopyFirstFrom(ValueAt(0),buffer_pool_manager); // 将第一个key和value复制到recipient
  IncreaseSize(-1);
}
